...
function pickIngredient(ingredient) {
    ...
    for (const flavorName in flavorIngredients) {
        const ingredients = flavorIngredients[flavorName];
    ...
    if (matchingFlavors.length > 0) {
        ...
        for (let i = 0; i < matchingFlavors.length; i++) {
            const name = matchingFlavors[i];
            const id = makeIdFromFlavor(name);
            html += `<li><a href="ingredients.html#${id}">${name}</a></li>`;
        }
        html += "</ul>";
...
function makeIdFromFlavor(name) {
    return (
        "flavor-" + name.toLowerCase().replace(/[^a-z0-9]+/g, "-")
    );
}
